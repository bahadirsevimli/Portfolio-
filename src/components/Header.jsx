...
export default function Header({
  scrollToSectionProjects,
  scrollToSectionSkills,
}) {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="flex sm:flex-row flex-col justify-between gap-3">
        <div className="w-15 h-15 bg-[#edebff] text-[#3730A3] text-3xl rounded-full pl-5 !pt-3 transform-[rotate(30deg)] ">
          <span className="opacity-70">B</span>
        </div>
        <nav className="flex  sm:gap-x-5 sm:gap-y-0 font-normal sm:text-lg text-sm">
          <div onClick={handleScrollSkills} className="!p-3 cursor-pointer">
            {data && data.headerSection.navbar.skills}
          </div>
          <div onClick={handleScrollProjects} className="!p-3 cursor-pointer">
            {data && data.headerSection.navbar.projects}
          </div>
          <Link to={data && data.headerSection.navbar.hireMeUrl}>
            <div className="border-1 border-[#3730A3] text-[#3730A3] sm:!p-3 sm:!pr-10 sm:!pl-10 p-2 rounded-xl ">
              {data && data.headerSection.navbar.hireMe}
            </div>
          </Link>
        </nav>
      </div>
